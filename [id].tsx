...
export default function PostDetailScreen() {
  ...
  const fetchPost = async () => {
    try {
      if (!id || typeof id !== 'string' || id === 'undefined' || id.length !== 36) {
  ...
  const handleToggleLike = async () => {
    if (!post?.post_id || !user?.id) {
      console.warn('⚠️ 좋아요 실패: post.post_id 또는 user.id가 없습니다.');
      return;
    }
  ...
  const handleDeletePost = async () => {
    if (!post?.post_id) {
      console.warn('⚠️ 삭제 실패: post.post_id가 없습니다.');
      return;
    }
  ...
  useEffect(() => {
    console.log('📌 [id].tsx 진입 시 id:', id);

    if (typeof id === 'string' && id.length === 36 && user) {
      fetchPost();
      fetchComments();
    }
  }, [id, user]);
    ...
    <ScrollView contentContainerStyle={styles.container}>
      ...
      {post.recipes && (
        <TouchableOpacity
          ...
          onPress={() => router.push(`/recipes/${post.recipes.id}`)}
        >
          {post.recipes.image_urls?.[0] && (
            <Image source={{ uri: post.recipes.image_urls[0] }} style={styles.recipeImage} />
          )}
          <Text style={styles.recipeTitle}>{post.recipes.title}</Text>
        </TouchableOpacity>
      )}
