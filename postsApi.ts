...
export async function getPosts() {
  const { data, error } = await supabase
    ...
    .select(`
      ...
      user_profiles:user_id ( id, display_name, avatar_url ),
      user_post_likes ( id ),
  ...
  return data.map(post => ({
    ...post,
    id: post.post_id, // 👈 Supabase에서 post_id를 id로 매핑
